...
/**
	...
	*/
class ScalaSQLQuery[A](c: org.hibernate.SQLQuery, s: MaintenanceModeAwareSession) {
	...
	def setLong(name: String, value: Long) = chainable {
		c.setLong(name, value)
	}

	def setTimestamp(name: String, value: java.sql.Timestamp) = chainable {
		c.setTimestamp(name, value)
	}
