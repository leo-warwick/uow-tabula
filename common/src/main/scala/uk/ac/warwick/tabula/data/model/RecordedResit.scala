...
import uk.ac.warwick.tabula.system.IntEnumTwoWayConverter
...
class RecordedResit extends GeneratedId
  ...
  @Type(`type` = "uk.ac.warwick.tabula.data.model.AttemptUserType")
  @Column(name = "current_resit_attempt", nullable = false)
  var currentResitAttempt: Attempt = _
...
// properties are vars so that spring form tags work
sealed abstract class Attempt(var value: Int, var name: String) extends IntEnumEntry
object Attempt extends IntEnum[Attempt] {
  val values: IndexedSeq[Attempt] = findValues

  case object FurtherFirstAttempt extends Attempt(value = 1, name = "Further first attempt")
  case object Resit extends Attempt(value = 2, name = "Resit")
  case object FinalResit extends Attempt(value = 3, name = "Final resit")
}

class AttemptUserType extends IntEnumUserType(Attempt)
class AttemptConverter extends IntEnumTwoWayConverter(Attempt)
